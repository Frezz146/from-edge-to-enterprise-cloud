...
- Python 3.11 or newer (verified with 3.14) or the .NET 8 SDK or newer (verified
  with .NET 10) for the language track you run. Like every C# project in this
  repository, `eval-runner` targets net8.0 and sets `RollForward=Major`, so it
  also runs on a machine that only has a newer runtime.
